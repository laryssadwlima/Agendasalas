...
// Receber os parâmetros de filtro
...
$query_events = "SELECT evt.id, evt.title, evt.color, evt.start, evt.end, evt.obs, evt.user_id, evt.client_id, 
                 usr.name AS user_name, cli.name AS client_name 
                 FROM events AS evt 
...
// Adicionar a condição para pesquisa por título ou nome do usuário
if (!empty($search)) {
    $conditions[] = "(evt.title LIKE :search OR usr.name LIKE :search)";
...
$query_events .= " ORDER BY evt.start ASC";
...
foreach ($params as $key => $value) {
    $result_events->bindParam($key, $value);
}
...
if ($result_events->execute()) {
    ...
    while ($row_events = $result_events->fetch(PDO::FETCH_ASSOC)) {
        ...
        $eventos[] = [
            ...
            'user_name' => $row_events['user_name'],
            'client_id' => $row_events['client_id'],
            'client_name' => $row_events['client_name']
        ];
    }
